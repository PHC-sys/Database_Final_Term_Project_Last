...
SELECT l.days, l.meal, m.initial_weight, end_weight, s.CustomerID,
...
/* Update_Wrong_EndWeight procedure 생성*/

DELIMITER //

CREATE PROCEDURE Update_Wrong_EndWeight()
BEGIN
...
INSERT INTO tfu 
SELECT l.days, l.meal, (l.end_weight - m.initial_weight) AS gap
FROM leftover AS l JOIN menu_management AS m
ON l.days = m.days
AND l.meal = m.meal
WHERE l.end_weight != 'IncorrectEnd_weight!';

UPDATE leftover
SET end_weight = "IncorrectEnd_weight!"
WHERE end_weight != 'IncorrectEnd_weight!'
AND (days, meal) IN (
SELECT days, meal
FROM tfu
WHERE gap >0);
